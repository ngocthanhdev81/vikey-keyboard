...
package dev.ngocthanhgl.vikey.lib.util
...
private const val IME_SERVICE_CLASS_NAME = "dev.ngocthanhgl.vikey.FlorisImeService"
private const val TIMED_QUERY_DELAY = 500L

object InputMethodUtils {
    fun isViKeyEnabled(context: Context): Boolean {
        return if (AndroidVersion.ATLEAST_API34_U) {
            ...
            enabledImeList != null && parseIsViKeyEnabled(context, enabledImeList)
        }
    }

    fun isViKeySelected(context: Context): Boolean {
        return if (AndroidVersion.ATLEAST_API34_U) {
            context.systemServiceOrNull(InputMethodManager::class)
            ...
            selectedIme != null && parseIsViKeySelected(context, selectedIme)
        }
    }

    @Composable
    fun observeIsViKeyEnabled(
        ...
        return if (AndroidVersion.ATLEAST_API34_U) {
            ...
            AndroidSettings.Secure.observeAsState(
                ...
                transform = { parseIsViKeyEnabled(context, it.toString()) },
            )
        }
    }

    @Composable
    fun observeIsViKeySelected(
        context: Context = LocalContext.current.applicationContext,
        foregroundOnly: Boolean = false,
    ): State<Boolean> {
        return if (AndroidVersion.ATLEAST_API34_U) {
            ...
            AndroidSettings.Secure.observeAsState(
                ...
                foregroundOnly = foregroundOnly,
                transform = { parseIsViKeySelected(context, it.toString()) },
            )
        }
    }

    fun parseIsViKeyEnabled(context: Context, activeImeIds: String): Boolean {
    ...
    fun parseIsViKeySelected(context: Context, selectedImeId: String): Boolean {
    ...
    private fun timedObserveIsFlorisBoardEnabled(): State<Boolean> {
        ...
        LaunchedEffect(Unit) {
            while (true) {
                state.value = isViKeyEnabled(context)
    ...
    private fun timedObserveIsFlorisBoardSelected(): State<Boolean> {
        ...
        LaunchedEffect(Unit) {
            while (true) {
                state.value = isViKeySelected(context)
                delay(TIMED_QUERY_DELAY)
            }
        }
        return state
