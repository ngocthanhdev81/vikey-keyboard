...
package dev.ngocthanhgl.vikey.app.devtools
...
import dev.ngocthanhgl.vikey.ime.keyboard.CachedLayout
import dev.ngocthanhgl.vikey.ime.keyboard.DebugLayoutComputationResult
    ...
    DevtoolsOverlayBox(title = "Failed to load stylesheet, fell back to base style") {
        ...
        if (cause is SnyggMissingSchemaException) {
            DevtoolsSubGroup(title = "Explanation") {
                DevtoolsText(
                    text = """
                    It appears you’re trying to load a theme designed for ViKey v0.4 (Snygg v1), which isn’t compatible with the latest release using Snygg v2.
